...
address:
  - code: University of Wollongong
    address: Wollongong Isotope Geochronology Laboratory, School of Earth & Envrionmental Sciences, University of Wollongong, Wollongong NSW Australia
...
library(knitr)
include_graphics(here::here("vignettes", "figures", "bone.png"))
```
...
ggsave(here::here("vignettes", "figures", "demo-panel-fig.png"))
```
...
include_graphics(here::here("vignettes", "figures", "demo-panel-fig.png"))
```
...
With this first run, we obtain an age of `r round(output_first_run_modern$T_final/1e3,1)` ka. There is no calculated error on the age since there is only one iteration. As we can see, the calculated (^234^U/^238^U) and (^230^Th/^238^U) ratios are higher than observed values (Figure \@ref(fig:plot-panel-second-run-modern-fig)). For the (^234^U/^238^U), it makes sense to thus use a narrower range for `U48_0_min` and `U48_0_max`. Higher calculated (^230^Th/^238^U) ratios compared to observed values suggests that the calculated age is too old (since this ratio increases with age). Thus, we should consider a lower value for `T_max`.
...
ggplot(output_first_run_modern$output_data) +
  ...
  xlab("Relative distance from center") +
  theme_plots +   
  ggtitle(paste("(234U/238U) at the\nsurface: ",
...
ggsave(here::here("vignettes", "figures", "plot-panel-first-run-modern.png"))
```
...
include_graphics(here::here("vignettes", "figures", "plot-panel-first-run-modern.png"))
```
...
ggplot(output_second_run_modern$output_data) +
  ...
  ylab(expression("("^234 * "U/"^238 * "U)")) +
  xlab("Relative distance from center") +
  theme_plots +   ggtitle(paste("(234U/238U) at the\nsurface: ",
...
The model is thus run a second time, increasing the number of iterations to 1000, and changing `U48_0_min` and `U48_0_max` to 1.265 and 1.270, respecitvely, and `T_max` to 10^3^ yr. Since the first run suggests a Holocene age for the sample, the measured (^234^U/^238^U) at the surfaces must be similar to the calculated values, thus the chosen values for the range above.
...
ggsave(here::here("vignettes", "figures", "plot-panel-second-run-modern.png"))
```
...
include_graphics(here::here("vignettes", "figures", "plot-panel-second-run-modern.png"))
```
...
`U48_0_min`, `U48_0_min`, `T_min` and `T_max` can be further adjusted. `fsum_target` can also be decreased to 0.01 in order to get a better fit and error, but it is at the expense of computing time. Once a satisfying fit is obtained (assessed by inspecting the figures produced), the number of iterations `nbit` can be increased to 1000 (or higher). Following this method, we obtain an age of `r age` +`r age_plus`/-`r age_minus` ka (Figure \@ref(fig:plot-panel-second-run-modern-fig)).
...
ggsave(here::here("vignettes", "figures", "plot-panel-hobbit.png"))
```
...
include_graphics(here::here("vignettes", "figures", "plot-panel-hobbit.png"))
```
