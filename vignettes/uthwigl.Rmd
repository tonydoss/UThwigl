...
`R28det` and `R28det_err` are the values for the (232Th/238U) activity ratio of the detritus and its standard error. Similarly, `R08det` and `R08det_err` are the values for the (230Th/238U) activity ratio of the detritus and its standard error, and `R48det` and `R48det_err` are the corresponding values for (234U/238U) activity ratio of the detritus.

# How to run the closed-system model

Attach the package as shown above and then run `csUTh()`, specifying the input data frame and the input parameters as described above. The code block below shows a quick example that will execute in less than five seconds on a typical 2.3 GHz Intel Core i5 laptop:  
