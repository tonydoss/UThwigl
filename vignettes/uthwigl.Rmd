...
Alternatively, the model can be run from the user's browser without installing R or RStudio with this [link](https://mybinder.org/v2/gh/benmarwick/UThwigl/master?urlpath=rstudio).
...
To use new data with this package, we need to import a CSV or Excel file with the U-Th data into the R environment using a generic function such as `read.csv` or `read_excel` from the `readxl` package [@Wickham_readxl]. The code chunk below shows how to read one of the CSV files included in the package into the R environment. This is a good method to use if the user supplies their own CSV file to use with `osUTh()`. In this case we assume that the user's working directory contains a directory called `data` and our CSV file is in this `data` directory: 
