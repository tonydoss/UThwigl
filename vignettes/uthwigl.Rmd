...
The x-y coordinates of each analysis location, the  inner and outer surfaces of the sample need to be recorded. One surface is given a coordinate of 1 and the other one -1, thus coordinates of analyses take values in between (Figure \@ref(fig:femurpic)).
...
write.csv(Hobbit_MH2T_for_iDAD, "Hobbit_MH2T_for_iDAD.csv")
```
...
input_data_os <- read.csv('/inst/extdata/input/Hobbit_MH2T_for_iDAD.csv')
```
...
The `x` and `y` columns corresponds to the coordinates (in mm) of the inner, outer surfaces and the analyses (Figure \@ref(fig:femurpic)). 
...
output_os$output_data %>%
  xtable(
         digits = c(3,3,3,3,3,1,1,1,0,3,3,3,3),
